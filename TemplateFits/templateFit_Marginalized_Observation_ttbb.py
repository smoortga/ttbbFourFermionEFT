...
# stolen function to determine minima/maxima
...
#             coupl_values = name.split("_")[2] + "_" + name.split("_")[4]
#             #if not coupl_values == "minus5p0_5p0": continue
...
#             observed_err_arr.append(2*observed_unc[v])
        ...
        print y_SM_obs,err_SM_obs,y_tL_obs,err_tL_obs,y_tR_obs,err_tR_obs
        y_SM_obs = func([obs1,obs2],*(coeff_SM))
        y_tL_obs = func([obs1,obs2],*(coeff_tL))
        y_tR_obs = func([obs1,obs2],*(coeff_tR))
        err_SM_obs = func([obs1,obs2],*(errors_SM))
        err_tL_obs = func([obs1,obs2],*(errors_tL))
        err_tR_obs = func([obs1,obs2],*(errors_tR))
        
        #chi2 = np.asarray([((y_SM_obs-i1)**2/err_SM_obs**2) + ((y_tL_obs-i2)**2/err_tL_obs**2) + ((y_tR_obs-i3)**2/err_tR_obs**2) for i1,i2,i3 in zip(y_SM,y_tL,y_tR) ])
        # chi2 with 2 dof
        chi2 = np.asarray([((y_tL_obs-i2)**2/err_tL_obs**2) + ((y_tR_obs-i3)**2/err_tR_obs**2) for i1,i2,i3 in zip(y_SM,y_tL,y_tR) ])
        #print chi2
        Y_chi2 = chi2.reshape(size)
        
        # find 4 minima
        im =  detect_peaks(Y_chi2)
        minima_indices = zip(*np.where(im == True))
        minima_x=[]
        minima_y=[]
        for idx,i in enumerate(minima_indices):
            minima_x.append(X1[i[0]][i[1]])
            minima_y.append(X2[i[0]][i[1]])

        
        levels = [5.991]#[3.841]
        CS = plt.contour(X1, X2, Y_chi2,levels,colors=('r'))
        plt.scatter(minima_x,minima_y)
        # levels = [3,5,10,20,50,100,200]
#         CS = plt.contourf(X1, X2, Y_chi2,levels)
        p_arr = CS.collections[0].get_paths()
        save_dict_gr = {}
        for idx,p in enumerate(p_arr): 
            v = p.vertices
            px = v[:,0]
            py = v[:,1]
            save_dict_gr[idx] = {"px":px,"py":py}
       ...
       # plt.show()
        ...
        pickle.dump(save_dict_gr,open("%s/contour_Marginalized_templateFit_Observation_%s.pkl"%(args.OutputDir,c),"wb"))
        print "saved contour in '%s/contour_Marginalized_templateFit_Observation_%s.pkl'"%(args.OutputDir,c)
    
        gr_dict = {}
        for idx,cont_dict in save_dict_gr.iteritems():
            px = cont_dict["px"]
            py = cont_dict["py"]
        
            gr_dict[idx] = ROOT.TGraph(len(px),array('d',px),array('d',py))
            gr_dict[idx].SetMarkerSize(0)
            gr_dict[idx].SetFillColor(2)
            gr_dict[idx].SetLineColor(2)
            gr_dict[idx].SetLineWidth(3)
        
        gr_minima = ROOT.TGraph(len(minima_x),array('d',minima_x),array('d',minima_y))
        gr_minima.SetMarkerStyle(5)
        gr_minima.SetMarkerSize(2)
        gr_minima.SetMarkerColor(2)
        gr_minima.SetLineWidth(2)
        
        gr_obs = ROOT.TGraph(1,array('d',[obs1]),array('d',[obs2]))
        gr_obs.SetMarkerStyle(34)
        ...
        for idx,gr in gr_dict.iteritems():
            mg.Add(gr,"lc")
        mg.Add(gr_minima,"p")
        mg.Add(gr_obs,"p")
        ...
        l.SetTextSize(0.05)
        print gr_dict
        l.AddEntry(gr_dict[0],"#splitline{template fit}{95% CL}","l")
        l.AddEntry(gr_minima,"best fit","p")
        l.AddEntry(gr_obs,"#splitline{input}{(%.1f,%.1f)}"%(obs1,obs2),"p")
        ...
        canvas.SaveAs('%s/fit_xsec_Marginalized_templateFit_Observation_%s.pdf'%(args.OutputDir,c))
    
        print "done, saved in '%s/fit_xsec_Marginalized_templateFit_Observation_%s.pdf'"%(args.OutputDir,c)
